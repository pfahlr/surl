...
pub fn from_regex(regex_str: &str) -> SlugPolicy {
  ...
  let (min_len, max_len) = re
    .and_then(|re| re.captures(regex_str).map(|c| (c[1].parse().ok(), c[2].parse().ok())))
    .and_then(|(a, b)| match (a, b) { (Some(x), Some(y)) => Some((x, y)), _ => None })
    .unwrap_or((5, 10));

  // Intersect with URL_SAFE
  let mut allow_chars: Vec<char> = URL_SAFE.chars().collect();
  // (For bootstrap we accept URL_SAFE outright; future task will refine by classes in regex.)
  SlugPolicy { allow_chars, min_len, max_len }
}
